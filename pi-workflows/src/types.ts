...
export interface WorkflowContext<P extends ProfileMap = ProfileMap> {
  ...
  readonly phaseLog: PhaseLog;
  readonly budget: BudgetTracker;
  phase(name: string): void;
  run<T>(name: string, fn: () => Promise<T> | T): Promise<T>;
  sh(name: string, command: string, options?: ShellOptions): Promise<ShellResult>;
  agent<T = string>(target: AgentTarget<P>, prompt: string, options?: AgentOptions<T>): Promise<T>;
  agentRaw<T = string>(target: AgentTarget<P>, prompt: string, options?: AgentOptions<T>): Promise<AgentResult<T>>;
